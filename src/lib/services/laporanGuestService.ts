...
// Helper function to convert type to Jabatan
function getJabatanFromType(type: string): Jabatan {
  return type.toUpperCase() === "ATM" ? "ATM" : "ABL";
}
...
export const getBarangTabsForLaporanGuest = cache(
  async (type: string): Promise<Array<{ id: number; nama: string }>> => {
    try {
      const jabatan = getJabatanFromType(type);
...
export const getLaporanStatsByPeriodeGuest = cache(
  async (type: string, periode: number = 1): Promise<LaporanStats> => {
    try {
      const jabatan = getJabatanFromType(type);
...
export const getChartLaporanByPeriodeGuest = cache(
  async (type: string, periode: number = 1): Promise<ChartLaporanData> => {
    try {
      const jabatan = getJabatanFromType(type);
      const currentDate = new Date();
      const labels: string[] = [];
...
export const getPembagianProvitByPeriodeGuest = cache(
  async (type: string, periode: number = 1): Promise<PembagianProvit[]> => {
    try {
      const jabatan = getJabatanFromType(type);
      const currentDate = new Date();
...
export const getTopPelangganByPeriodeGuest = cache(
  async (
    type: string,
...
export const getChartBarangLaporanByPeriodeGuest = cache(
  async (type: string, periode: number = 1): Promise<ChartBarangItem[]> => {
    try {
      const jabatan = getJabatanFromType(type);
      const { startDate, endDate } = getDateRangeByPeriode(periode);
